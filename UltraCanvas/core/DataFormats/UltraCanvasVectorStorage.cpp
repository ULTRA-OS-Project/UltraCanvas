...
// Skips SVG's separators: whitespace and at most commas between them.
void SkipSvgSeparators(const char*& p, const char* end) {
    while (p < end && (std::isspace(static_cast<unsigned char>(*p)) || *p == ',')) ++p;
}

// One number per the SVG grammar ("-.5", "1e-3", "+2"), dot-decimal whatever
// the locale. The number ends where the next one starts, so "1.5-2.5.5"
// reads as 1.5, -2.5, .5. Returns false (p unchanged) when none is there.
bool ScanSvgNumber(const char*& p, const char* end, float& out) {
    SkipSvgSeparators(p, end);
    const char* q = p;
    if (q < end && (*q == '+' || *q == '-')) ++q;
    bool digits = false;
    while (q < end && std::isdigit(static_cast<unsigned char>(*q))) { ++q; digits = true; }
    if (q < end && *q == '.') {
        ++q;
        while (q < end && std::isdigit(static_cast<unsigned char>(*q))) { ++q; digits = true; }
    }
    ...
    if (q < end && (*q == 'e' || *q == 'E')) {
        ...
        if (e < end && std::isdigit(static_cast<unsigned char>(*e))) {
            while (e < end && std::isdigit(static_cast<unsigned char>(*e))) ++e;
            q = e;
        }
    }
    // from_chars parity: no leading '+', which SVG allows ("M+10+20").
    ParseFloatClassic(*p == '+' ? p + 1 : p, q, out);
    p = q;
    return true;
}
...
// Reads SVG path data by the grammar in SVG 1.1 section 8.3.9, not by
// whitespace-separated tokens. Real files (Illustrator, Inkscape, svgo) write
// the compact form: numbers run together wherever the next one starts with a
// sign or a second decimal point ("423.38-18.759", ".95-.16.857"), commas and
// spaces are interchangeable, a command letter is written once and followed
// by as many coordinate sets as it has (an extra pair after M/m is an
// implicit L/l), and the arc flags are single digits that need no separator
// ("a1 1 0 01 5 5"). The old stream-based reader handled none of that and
// turned such paths into runs of empty commands, which is why most shapes of
// an optimised SVG vanished on import.
PathData ParsePathString(const std::string& pathStr) {
    PathData result;
    const char* p = pathStr.c_str();
    const char* const endp = p + pathStr.size();

    auto skipSeparators = [&]() { SkipSvgSeparators(p, endp); };
    auto readNumber = [&](float& out) -> bool { return ScanSvgNumber(p, endp, out); };
    // Arc flags are one character, '0' or '1', with no separator required.
    auto readFlag = [&](float& out) -> bool {
        skipSeparators();
        if (p < endp && (*p == '0' || *p == '1')) { out = (*p == '1') ? 1.0f : 0.0f; ++p; return true; }
        return false;
    };
    ...
    while (true) {
        ...
        if (std::isalpha(static_cast<unsigned char>(*p))) {
            ...
            break;   // data must start with a command
        }
        // Otherwise: more coordinates for the current command (implicit repeat).
        ...
        int count = 0;
        switch (up) {
            case 'M': pathCmd.Type = PathCommandType::MoveTo;            count = 2; break;
            case 'L': pathCmd.Type = PathCommandType::LineTo;            count = 2; break;
            case 'H': pathCmd.Type = PathCommandType::HorizontalLineTo;  count = 1; break;
            case 'V': pathCmd.Type = PathCommandType::VerticalLineTo;    count = 1; break;
            case 'C': pathCmd.Type = PathCommandType::CurveTo;           count = 6; break;
            case 'S': pathCmd.Type = PathCommandType::SmoothCurveTo;     count = 4; break;
            case 'Q': pathCmd.Type = PathCommandType::QuadraticTo;       count = 4; break;
            case 'T': pathCmd.Type = PathCommandType::SmoothQuadraticTo; count = 2; break;
            case 'A': pathCmd.Type = PathCommandType::ArcTo;             count = 7; break;
            case 'Z':
                pathCmd.Type = PathCommandType::ClosePath;
                result.Closed = true;
                result.commands.push_back(pathCmd);
                cmd = 0;   // Z takes no coordinates; a number after it is an error
                continue;
            default:
                return result;   // unknown command: keep what was read so far (SVG error handling)
        }

        pathCmd.Parameters.resize(static_cast<size_t>(count));
        bool ok = true;
        for (int i = 0; i < count && ok; ++i) {
            ok = (up == 'A' && (i == 3 || i == 4)) ? readFlag(pathCmd.Parameters[static_cast<size_t>(i)])
                                                   : readNumber(pathCmd.Parameters[static_cast<size_t>(i)]);
        }
        if (!ok) break;   // truncated or malformed: render up to the last complete segment
        result.commands.push_back(pathCmd);
        // Coordinates repeated after a moveto are implicit linetos.
        if (up == 'M') cmd = relative ? 'l' : 'L';
    }
    return result;
}
...
Matrix3x3 ParseTransformString(const std::string& transformStr) {
    ...
    while (it != end) {
        ...
        // Arguments are separated by whitespace, commas or both. The old
        // reader skipped "the comma" by reading one character after every
        // number, which with a space separator ate the first digit of the
        // next one: translate(483.572 574.049) became (483.572, 74.049) and
        // scale(1 -1) lost its sign.
        std::vector<float> values;
        const char* p = params.c_str();
        const char* const pe = p + params.size();
        float val = 0.0f;
        while (ScanSvgNumber(p, pe, val)) values.push_back(val);
        ...
        } else if (func == "rotate" && values.size() >= 3) {
            // rotate(a cx cy) turns about (cx, cy), not the origin.
            result = result * Matrix3x3::Translate(values[1], values[2]) *
                     Matrix3x3::RotateDegrees(values[0]) *
                     Matrix3x3::Translate(-values[1], -values[2]);
