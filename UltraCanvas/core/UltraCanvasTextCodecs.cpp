// core/UltraCanvasTextCodecs.cpp
// The Base64 / Base32 codecs, split out of UltraCanvasTextUtils.cpp.
//
// They are a separate translation unit — and a separate static library — for a
// linkage reason, not a stylistic one. The string helpers next door are linked
// into libultracanvas, so a SHARED core exports them; UltraCrypt needs only the
// codecs. Keeping both groups in one object meant that linking UltraCrypt (for
// Base32) also dragged in the object's copy of Trim() and friends, which then
// collided with the same symbols exported by the shared core — a hard link
// error on PE/COFF ("multiple definition of UltraCanvas::Trim"). Split, each
// symbol is compiled once and reachable from exactly one place: the strings
// from the core, the codecs from this archive.
//
