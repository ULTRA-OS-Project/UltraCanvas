...
public:
    ...
    static std::vector<Point2D> ParsePoints(const std::string& pointsStr);
    static std::vector<Point2D> ParsePath(const std::string& pathStr);
};
...
public:
    SVGElementRenderer(const SVGDocument& doc) : document(doc) {}
    
    void RenderElement(const SVGElement& element);
    void RenderDocument(const Rect2D& viewport);
};
...
public:
    ...
    void Render(const Rect2D& bounds) override;
