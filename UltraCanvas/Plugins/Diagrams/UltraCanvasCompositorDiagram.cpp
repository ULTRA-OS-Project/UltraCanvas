...
UltraCanvasCompositorDiagram::ComputeNodeLayout(
    ...
    if (tmpl.isGroup) {
        ...
        double maxXb = L.bounds.x + std::max<double>(L.bounds.width, tmpl.groupMinWidth);
        double maxYb = L.bounds.y + std::max<double>(L.bounds.height, tmpl.groupMinHeight);
        for (const auto& kv : nodes) {
...
void UltraCanvasCompositorDiagram::Render(IRenderContext* ctx, const Rect2Df& dirtyRect) {
    (void)dirtyRect;
...
void UltraCanvasCompositorDiagram::RenderRowWidget(
    ...
    switch (spec.widget) {
        ...
        case ParamWidgetKind::Checkbox: {
            double box = std::min<double>(wb.height, 14.0);
            ...
            switch (spec->widget) {
                ...
                case ParamWidgetKind::Slider: {
                    ...
                    for (size_t i = 0; i < tmpl->params.size() && i < L.rowWidgetBounds.size(); ++i) {
                        ...
                        const auto& wb = L.rowWidgetBounds[i];
                        Point2Df w = ScreenToWorld(mousePos);
                        double t = (w.x - wb.x) / std::max<double>(1e-6, wb.width);
                        t = std::max(0.0, std::min(1.0, t));
