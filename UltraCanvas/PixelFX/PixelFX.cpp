...
// ============================================================================
    namespace Draw {
        ...
        void FloodFillTolerance(PFXImage& image, int x, int y, const std::vector<double>& ink, double tolerance, FloodMatch match) {
            ...
            // vips must OWN a copy of the pixels: plain new_from_memory only wraps
            // the caller's buffer, and copy_memory() on an already-memory image
            // just adds a reference — so the result would dangle onto this local
            // `pix` vector once the function returns and be read (freed) later when
            // the pipeline is evaluated. Call the C vips_image_new_from_memory_copy
            // (which copies) directly rather than the C++ VImage::new_from_memory_copy
            // wrapper, which older libvips (e.g. 8.12) don't expose; VImage steals
            // the returned reference.
            image = PFXImage(vips::VImage(vips_image_new_from_memory_copy(
        ...
        PFXImage MagicWandMask(const PFXImage& image, int x, int y, double tolerance, FloodMatch match) {
            ...
            // vips must own the mask pixels; plain new_from_memory + copy_memory()
            // would leave the returned image pointing at this local `mask` buffer
            // (copy_memory only refs an already-memory image), which is then read
            // after free when the Ifthenelse pipeline is evaluated — a garbled mask
            // on Linux and a worker-thread segfault on Windows. Use the C
            // vips_image_new_from_memory_copy (which copies) instead of the C++
            // VImage::new_from_memory_copy wrapper, absent in older libvips (e.g.
            // 8.12); VImage steals the returned reference.
            return PFXImage(vips::VImage(vips_image_new_from_memory_copy(
