...
// ===== SIMPLE SVG DOM STRUCTURES =====
    ...
    public:
        std::vector <SVGPathCommand> pathCommands;
        SVGPathElement(const std::string& tag) : SVGElement(tag) {}
    };
...
// ===== LIGHTWEIGHT SVG PARSER =====
    ...
    public:
        ...
        static void ParsePathCommands(const std::string& pathStr, std::vector<SVGPathCommand>& commands);
    };
...
// ===== SVG ELEMENT RENDERER =====
    ...
    private:
        ...
        void ExecutePathCommands(IRenderContext* ctx, const std::vector<SVGPathCommand>& commands);
        void ConvertArcToCubicBezier(IRenderContext* ctx, const Point2Df& start,
                                     float rx, float ry, float xAxisRotation,
                                     bool largeArc, bool sweep, const Point2Df& end);
        void ApplyFillStyles(const SVGElement& element);
        void ApplyStrokeStyles(const SVGElement& element);
