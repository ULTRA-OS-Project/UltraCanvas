...
data model, API, broker, adapters, security, delivery plan, and §12 on how
UltraScript and the channel share one dictionary).
...
`com-automation`, at-rest journal encryption via UltraCrypt. **UltraScript**
(`Docs/Research/UltraScriptSpecification.md`: the AppleScript-class language,
its recorder, SDEF-compatible dictionary, executor and Script Editor) is a
client of the Phase 3 command surface: an app's scripting dictionary is its
UltraMessage command manifest, and a `tell application "X"` block whose
target is another process travels over this channel as `ui.*` / app verbs.
