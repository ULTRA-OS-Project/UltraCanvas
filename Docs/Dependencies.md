...
| Zigbee backend (`ULTRACANVAS_SMARTHOME_ZIGBEE`, default OFF) | Silicon Labs EZSP **or** TI Z-Stack ZNP | same | same |
| Z-Wave backend (`ULTRACANVAS_SMARTHOME_ZWAVE`, default OFF) | OpenZWave (**LGPL 2.1**) | OpenZWave | OpenZWave |
| KNX backend (`ULTRACANVAS_SMARTHOME_KNX`, default OFF) | (core only) — KNXnet/IP over sockets | (core only) | (core only) |
