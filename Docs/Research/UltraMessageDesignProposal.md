...
`ultramail` and the `UltraCanvasMessageCenter` element (§11) are built. The
...
boundary, its §17 says how it uses the channel
...
| **C. Commands** | An app registers verbs with typed arguments; anyone may list and invoke them, with the user's consent | **UltraScript** (`UltraScriptSpecification.md` §17) for repeating tasks across apps; UltraAI agents driving apps. A minimal hook: register, list, invoke, consent — no language features live here |
...
- Not a scripting language, and not the home of anything scripting-shaped.
  UltraScript is its own module (`Masterfile_modules.md` §14): it parses and
  runs scripts, records user actions and owns the in-process object model of
  windows and elements. When one of its scripts reaches into *another*
  application, the verbs it calls are ordinary UltraMessage commands (§6);
  that is the whole of the coupling, and it runs one way (§12).
...
| `app.command.echo` | Notice | no (`Silent`) | `caller`, `target`, `verb`, `args`, `ok` — the broker's echo of every invocation it routed, delivered only to endpoints the user granted the *recorder* role; what a cross-application recorder subscribes to |
...
(`<app>.sdef`, shipped next to the binary or inside the bundle). There is
exactly one description of what an app can do; UltraMessage reads it as its
command manifest, `UltraMsg_ListCommands` returns it rendered to JSON
(suites → classes → properties and commands, with `UltraMsgCommandInfo` for
each command), and the macOS adapter (§9.3) hands the same file to the
system unchanged. The broker indexes dictionaries on start, the same way
"Open with" indexes handlers today. An app that does not link UltraScript
may ship a dictionary written by hand, or none, and register commands
through `UltraMsg_RegisterCommand` alone; the format is shared, the
generator is UltraScript's.
...
| **apple-events-export** | in | Register an `NSAppleEventManager` handler for the UltraCanvas suites and ship the app's dictionary as its `sdef` (§6.4), so **AppleScript and Shortcuts can drive UltraCanvas apps** (`tell application "UltraViewer" to export …`); each incoming Apple Event becomes an `app.command.invoke` on the local endpoint, whatever verbs the app registered | Symmetry with dbus-export |
...
launcher, or a DemoApp page. *Built:* `include/Plugins/UltraMessage/UltraCanvasMessageCenter.h`,
documented in `Docs/UltraCanvas/UltraCanvasMessageCenter.md`; the segmented
control stands in for the tabbed container named above, since the four
sections filter one list rather than hold four.

---

## 12. Relationship to UltraScript and to AI agents

[`UltraScriptSpecification.md`](UltraScriptSpecification.md) is the
scripting language of the ULTRA OS stack: three syntax styles, an automatic
recorder, an SDEF-compatible dictionary, a parser and executor, and a Script
Editor. It is a **separate module** (`Masterfile_modules.md` §14) and a
**client** of this one. The two meet on one primitive — a per-user broker
with app identity, addressed request and reply, and subscriptions — and on
nothing else. Building that primitive twice would be the mistake; putting
language features into the channel would be the other one.

### 12.1 The boundary

| | UltraScript owns | UltraMessage owns |
|---|---|---|
| Language | Syntax styles, parser, AST, executor, standard library, debugger, Script Editor | — |
| Object model | Windows and elements *inside one process*: names, classes, properties, commands, `IUltraScriptable`; the generic verbs that expose that model to other processes | The application as a whole: app ids, instances, launch |
| Dictionary | Its format (SDEF XML) and its generation from element metadata | Its distribution: indexed per installed app, served by `UltraMsg_ListCommands`, exported to D-Bus introspection and to macOS (§6.4) |
| Recording | The in-process recorder and any cross-application recording | The `app.command.echo` topic it subscribes to (§5.3) |
| Repeating tasks | Message-triggered handlers and schedules in scripts | The subscription API those are built on |
| Crossing a process | — | Transport, delivery guarantees, consent, journal, launching the target |

**Dependency direction.** UltraScript links UltraMessage and needs its
Phases 1 and 3. UltraMessage never links UltraScript; the feed (Phase 2)
needs nothing from Phase 3. An UltraScript-linked application is an ordinary
endpoint that registers ordinary commands.

### 12.2 What the channel guarantees to any command client

Three facts, defined here because they are channel facts; how UltraScript
builds on them is its specification's §17.

1. **One dictionary.** The scripting dictionary is the command manifest
   (§6.4). An app author describes a command once and it is listable and
   invokable in-process, over the channel, from D-Bus and from AppleScript.
2. **The `app.command.*` topics** (§5.3): `list`, `invoke` with a reply,
   and the broker's `echo` of routed invocations for endpoints holding the
   recorder role. A client that wants to expose a richer model — UltraScript's
   `ui.*` verbs, for instance — registers them as commands like any other;
   the channel does not know what they mean.
3. **Consent and identity** (§10). Invoking another application's command is
   granted by the user once per caller and target and can be revoked there.
   The broker verifies the caller's executable, so a consent granted to the
   Script Editor does not extend to a process merely claiming its name.
   Subscribing, including to `app.command.echo`, is a role the user grants
   in the same place.

### 12.3 What is deliberately not here

No `ui.*` verbs, no object paths, no triggers, no scheduler, no recorder.
Each was drafted in this document first and moved to
`UltraScriptSpecification.md` §17, where its owner is. The feed must not
tempt the language into messenger verbs either: "reply to this Telegram
message" is a notification action (§5.3, `system.notification.action`)
handled by the adapter that produced it, never a language feature.
...
is a tool list, `UltraMsg_Invoke` is a tool call, and the journal export is
context. When several steps belong together, an agent may write an
UltraScript script and hand it to that module's executor; the channel sees
only the resulting invocations.
...
tests a way to drive apps; Phase 2 is the desktop feed and the reason the
module exists; Phase 3 is the hook UltraScript's cross-application scripting
(its §17) and AI agents plug into. The feed never waits on Phase 3.
