...
| **C. Commands** | An app registers verbs with typed arguments; anyone may list and invoke them | Repeating tasks across apps (the AppleScript use case); UltraAI agents driving apps; **UltraScript** (`UltraScriptSpecification.md`), whose `tell application "X"` blocks cross the process boundary on this surface |
...
- Not a scripting language. UltraScript parses and runs scripts, records user
...
the SDEF-compatible XML that `UltraScriptSpecification.md` §6 defines
...
// (A UltraScript script will write this line; today it is C++.)
JSONValue out;
...
Events** (what AppleScript is made of). The UltraScript specification's
...
| **apple-events-export** | in | Register an `NSAppleEventManager` handler for the UltraCanvas suites and ship the app's UltraScript dictionary as its `sdef` (the specification chose the SDEF schema for exactly this), so **AppleScript and Shortcuts can drive UltraCanvas apps** (`tell application "UltraViewer" to export …`); incoming Apple Events become `ui.*` / app verbs on the local endpoint (§12.2) | Symmetry with dbus-export; the "macOS bridging (NSAppleScript compatibility)" file the specification lists under `OS/MacOS/` |
...
[`UltraScriptSpecification.md`](UltraScriptSpecification.md) (v1.0.0, 2024-12-19) is the ULTRA OS answer to
...
**scripting dictionary**, an `IUltraScriptable` interface every element gets
...
| Object model | Windows and elements *inside one process*: names, classes, properties, commands, `IUltraScriptable` | The application as a whole: app ids, instances, launch, and verbs that are not tied to an element (`export`, `send`, `search`) |
| Recording | `UltraScriptRecorder` on the in-process event stream | An echo of cross-app invocations the broker routed, so a recording can span applications (§12.4) |
...
Every application that links UltraScript registers four generic commands on its
endpoint at start-up, implemented by the UltraScript executor itself:
...
When `UltraScriptExecutor` resolves `tell application "UltraViewer"` and that is
...
`UltraScriptResult` with the script line number. Launching an installed but
...
(`UltraScriptStandardLibrary::RegisterCommand("sendEmail", …)`, §13.2) and this
module's `UltraMsg_RegisterCommand` describe the same thing. They should be
one call: the UltraScript registry is implemented as a thin wrapper that also
...
`UltraScriptRecorder` sees only its own process. For a recording that spans
...
### 12.7 Implementation note for UltraScript
...
a value-change hook on `UltraCanvasUIElement`; this is a UltraScript decision,
...
context, and a UltraScript script is what an agent may write and hand to the
...
| **3 — Commands** | `RegisterCommand`/`ListCommands`/`Invoke`, dictionaries as manifests, consent store and prompt, the `ui.*` verbs and the executor's cross-application routing (§12.2, jointly with UltraScript), `app.command.echo` for cross-app recording, dbus-export + dbus-invoke, apple-events-invoke + apple-events-export, copydata; UltraFiler/UltraViewer/UltraMail register their first verbs (`open`, `export`, `send`) | Phase 1; UltraScript's dictionary and executor |
| **4** | Android listener, WASM transport, apple-messages, matrix, imap-idle, com-automation; at-rest journal encryption; `on message` triggers and schedules for UltraScript scripts (§12.5) | Phases 2–3 |
...
AppleScript-class automation, and is where UltraScript's own Phase 1 (recorder,
...
1. **Name.** `UltraMessage` with the `UltraMsg_` prefix (as `UltraDb_` for
   ...
   vocabulary in the feed). The language is UltraScript, matching the
   module family.
