...
**Companion:** [UIScriptSpecification.md](UIScriptSpecification.md) — the
scripting language (recorder, dictionary, parser, executor, editor) whose
cross-application scripting rides on this channel; §12 reconciles the two
...
| **C. Commands** | An app registers verbs with typed arguments; anyone may list and invoke them | Repeating tasks across apps (the AppleScript use case); UltraAI agents driving apps; **UIScript** (`UIScriptSpecification.md`, the language some notes call UltraScript), whose `tell application "X"` blocks cross the process boundary on this surface |
...
- Not a scripting language. UIScript parses and runs scripts, records user
  actions and owns the in-process object model of windows and elements; the
  *verbs* it calls in another application are UltraMessage commands (§6).
  Designing the channel to the specification's dictionary means the language
  gets every app for free and needs no second transport (§12).
...
### 6.4 App manifest: the scripting dictionary
...
with one key, `X-UltraMessage-AppId`, plus its **scripting dictionary**:
the SDEF-compatible XML that `UIScriptSpecification.md` §6 defines
(`<app>.sdef`, generated by `GenerateScriptingDictionary()` and shipped next
to the binary or inside the bundle). There is exactly one description of
what an app can do; UltraMessage reads it as its command manifest,
`UltraMsg_ListCommands` returns it rendered to JSON (suites → classes →
properties and commands, with `UltraMsgCommandInfo` for each command), and
the macOS adapter (§9.3) hands the same file to the system unchanged. The
broker indexes dictionaries on start, the same way "Open with" indexes
handlers today.
...
// (A UIScript script will write this line; today it is C++.)
JSONValue out;
...
Events** (what AppleScript is made of). The UIScript specification's
AppleScript heritage was about repeating tasks *in* applications; that is
job C, and it is the same transport.
...
| **apple-events-export** | in | Register an `NSAppleEventManager` handler for the UltraCanvas suites and ship the app's UIScript dictionary as its `sdef` (the specification chose the SDEF schema for exactly this), so **AppleScript and Shortcuts can drive UltraCanvas apps** (`tell application "UltraViewer" to export …`); incoming Apple Events become `ui.*` / app verbs on the local endpoint (§12.2) | Symmetry with dbus-export; the "macOS bridging (NSAppleScript compatibility)" file the specification lists under `OS/MacOS/` |
...
## 12. Relationship to UIScript and to AI agents

[`UIScriptSpecification.md`](UIScriptSpecification.md) (v1.0.0, 2024-12-19;
referred to as UltraScript in some notes) is the ULTRA OS answer to
AppleScript: a language in three syntax styles (Modern, Natural, Classic), an
automatic **recorder** hooked into event dispatch, an SDEF-compatible XML
**scripting dictionary**, an `IUIScriptable` interface every element gets
almost for free, a parser and executor, and a Script Editor application with
a dictionary browser. Its object model is `Application → Window → UIElement`,
addressed by script name and class. Nothing of it is implemented yet (no
branch carries any of its files), and its Phase 2 "remote scripting" and
Phase 3 "cross-application scripting" are left open. Those two items are
what this module supplies. The split:

| | UIScript owns | UltraMessage owns |
|---|---|---|
| Language | Syntax styles, parser, AST, executor, standard library, debugger | — |
| Object model | Windows and elements *inside one process*: names, classes, properties, commands, `IUIScriptable` | The application as a whole: app ids, instances, launch, and verbs that are not tied to an element (`export`, `send`, `search`) |
| Recording | `UIScriptRecorder` on the in-process event stream | An echo of cross-app invocations the broker routed, so a recording can span applications (§12.4) |
| Dictionary | Its format (SDEF XML) and generation from element metadata | Its distribution: indexed per installed app, served by `UltraMsg_ListCommands`, exported to D-Bus introspection and to macOS |
| Crossing a process | — | Transport, delivery guarantees, consent, journal, launching the target |
| Editor | The Script Editor app | The message centre; the `ultramsg` CLI |

### 12.1 One dictionary

The specification's dictionary *is* the command manifest (§6.4). A class's
commands and properties become `UltraMsgCommandInfo` entries; the
application-level suite becomes the app's own verbs. `UltraMsg_ListCommands`
and the Script Editor's dictionary browser read the same file, so an app
author writes `RegisterScriptProperty` / `RegisterScriptCommand` once and is
scriptable in-process, over the bus, from D-Bus and from AppleScript.

### 12.2 The `ui.*` verbs: the object model over the channel

Every application that links UIScript registers four generic commands on its
endpoint at start-up, implemented by the UIScript executor itself:

| Verb | Arguments | Reply |
|---|---|---|
| `ui.list` | `path` (object path), `class?` | The children matching, as `{class, name, index}` |
| `ui.get` | `path`, `property` | The value |
| `ui.set` | `path`, `property`, `value` | ok / error |
| `ui.invoke` | `path`, `command`, `args[]` | The command's result |

An **object path** is the specification's object reference serialised as a
JSON array from the application down: `window "Main"` → `button "OK"` is
`[{"class":"Window","name":"Main"},{"class":"Button","name":"OK"}]`; an
index reference uses `"index"` instead of `"name"`. Values cross as
`JSONValue` with the specification's types mapped one to one: `string`,
`integer`, `real`, `boolean` natively; `color`, `point`, `rect` through the
existing `JSON::FromColor` / `FromPoint` / `FromRect` helpers; `list` and
object references as arrays and paths.

When `UIScriptExecutor` resolves `tell application "UltraViewer"` and that is
not the running process, it does not fail: it sends each statement inside the
block as an `app.command.invoke` request carrying the matching `ui.*` verb (or
the app's own verb when the statement names one from the application suite),
waits for the reply within the script's timeout, and maps an error reply onto
`UIScriptResult` with the script line number. Launching an installed but
stopped target uses `UltraMsg_Invoke`'s `launch` option. This is the
specification's cross-application scripting with no second transport, and
its "remote scripting" is the same thing with a broker reached over UltraNet
later.

### 12.3 Application-level commands and consent

The specification's global command registry
(`UIScriptStandardLibrary::RegisterCommand("sendEmail", …)`, §13.2) and this
module's `UltraMsg_RegisterCommand` describe the same thing. They should be
one call: the UIScript registry is implemented as a thin wrapper that also
registers on the endpoint, so a verb is available to a local script and to
every other process alike.

The specification's security section asks for a sandbox and for permission
before scripts act. In-process scripts need no consent; a script driving
*another* application goes through the consent store of §10, once per
caller / target pair, which is also where a user revokes it. The broker
verifies the caller's identity, so a consent granted to the Script Editor
does not extend to an arbitrary process claiming its name.

### 12.4 Recording across applications

`UIScriptRecorder` sees only its own process. For a recording that spans
applications, the broker publishes a `Silent`, non-journaled echo of every
`app.command.invoke` it routed on `app.command.echo` (caller, target, verb,
args), visible only to endpoints the user granted the *recorder* role. The
Script Editor subscribes and emits `tell application "X" … end tell` blocks
in the chosen syntax style, interleaved with the local recording by
timestamp. Optional, Phase 3.

### 12.5 Repeating tasks and message-driven scripts

The specification has `delay()` but no scheduler and no way to react to
something happening in another application. Two small additions close the
"repeating tasks in apps" use case without changing the language's shape:

- **Triggers.** A script may declare handlers for topics:
  `on message "mail.message" (msg) … end message` (Natural / Classic) or
  `onMessage("mail.message", function(msg) { … })` (Modern). The executor
  subscribes through `UltraMsg_Subscribe` and runs the handler with the
  message body as an object. A script becomes a rule: "when a mail from
  the accountant arrives, export the attachment into UltraFiler's Invoices
  folder".
- **Schedules.** The ULTRA OS shell (or the `ultramsg schedule` sub-command
  elsewhere) runs a script at a time or interval and on log-in, keeping the
  schedule in the journal database. Every scheduled run is itself journaled
  on `script.run` so the message centre can show what ran and what failed.

### 12.6 Mapping

| AppleScript | UIScript | UltraMessage |
|---|---|---|
...
| Scripting dictionary (`sdef`) | The dictionary (SDEF XML), generated from element metadata | The same file, indexed per app; `UltraMsg_ListCommands` |
| `tell application "X"` | Resolved to the current application | Routed to `org.example.x`, launching if needed |
| Object specifier (`button "OK" of window "Main"`) | Element resolved by name and class | Object path in a `ui.*` verb |
| Automation consent | "Request permission before recording / executing" | `consents` table + prompt (§10), per caller and target |
| Folder actions / `on idle` | `delay()` | `on message` handlers and schedules (§12.5) |
| `osascript` | The Script Editor | `ultramsg` CLI: `post`, `invoke`, `query`, `tail`, `schedule` |

### 12.7 Implementation note for UIScript

The specification names the hooks it needs by names that do not exist in the
codebase today: it wires the recorder into
`UltraCanvasWindow::DispatchEventToChildren` / `FindEventTarget`, and it
records `UCEventType::ValueChanged` and `SelectionChanged`. The real dispatch
lives in `UltraCanvasApplication::DispatchEvent` /
`DispatchEventToElement`, and `UCEventType` has no value or selection
events — those are element callbacks (`onValueChanged`, `onSelectionChanged`
on the individual widgets). The recorder therefore needs either a
`NotifyScriptableChange` call from the elements that fire such callbacks or
a value-change hook on `UltraCanvasUIElement`; this is a UIScript decision,
noted here because the `ui.*` verbs of §12.2 rely on the same property
plumbing.

### 12.8 AI agents

The same command surface is what an UltraAI agent uses to act on the user's
behalf (`Docs/AI-Agent-Integration.md`, surface B): `UltraMsg_ListCommands`
is a tool list, `UltraMsg_Invoke` is a tool call, the journal export is
context, and a UIScript script is what an agent may write and hand to the
executor when several steps belong together.
...
| **3 — Commands** | `RegisterCommand`/`ListCommands`/`Invoke`, dictionaries as manifests, consent store and prompt, the `ui.*` verbs and the executor's cross-application routing (§12.2, jointly with UIScript), `app.command.echo` for cross-app recording, dbus-export + dbus-invoke, apple-events-invoke + apple-events-export, copydata; UltraFiler/UltraViewer/UltraMail register their first verbs (`open`, `export`, `send`) | Phase 1; UIScript's dictionary and executor |
| **4** | Android listener, WASM transport, apple-messages, matrix, imap-idle, com-automation; at-rest journal encryption; `on message` triggers and schedules for UIScript scripts (§12.5) | Phases 2–3 |
...
AppleScript-class automation, and is where UIScript's own Phase 1 (recorder,
parser, executor, editor) and this module meet.
...
1. **Name.** `UltraMessage` with the `UltraMsg_` prefix (as `UltraDb_` for
   ...
   vocabulary in the feed). The language keeps the specification's name,
   UIScript, unless it is renamed UltraScript to match the module family;
   this document follows whichever the specification says.
