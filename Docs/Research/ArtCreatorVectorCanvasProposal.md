...
| **4. Xara-class effects** — *done in 0.8.84 / ArtCreator 0.2.0* | effects in the model, per-object raster caches with blur, shadow / feather / transparency tools, multi-stage fills, XAR round trip of effects, unify the two XAR readers (the converter now reads through the XAR plugin's `XARDocument`), arrowheads / brushes / variable width (Xara's own arrowheads are XAR line attributes - their default reference numbers follow Xara LX and are unverified against a Designer export; the rest bakes into XAR as plain shapes under a `TAG_USERVALUE` marker and reads back as the stroke) | ~4 k lines | 1, 3 |
