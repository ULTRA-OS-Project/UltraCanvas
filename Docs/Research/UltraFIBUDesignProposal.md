...
|---|---|---|
| **B4** *(driver ✅, TLS ✅, vault credentials ✅, placeholder rewriting ✅ tested; pooling open)* | `libpq` PostgreSQL driver for UltraDatabase behind `IUltraDbDriverPlugin`: TLS defaults to `verify-full`, credentials must be a `vault:` key and a literal password is refused, SQLSTATE mapped to result codes, `?` rewritten to `$n` outside literals, comments and dollar-quoted bodies. **Two driver hooks turned out to be necessary rather than cosmetic: `BeginTransactionSql()` and `RowLockSuffix()` — SQLite serialises writers and PostgreSQL at READ COMMITTED does not.** Still open: connection pooling | The UltraDatabase suite passes unchanged against both, and UltraFIBU's full schema migrates and posts on a real server ✅ |
...
| **A2** *(engine ✅, PDF ✅, screens ✅ read-only, tax-treatment check ✅, key selection ✅; **document entry dialog open**)* | Documents and the journal: `Beleg`/`BelegPosition`/`Buchung`, posting with the automatic tax split, *Storno*, payments, **Festschreibung** reaching the rows, the hash chain and its verifier, the Summen- und Saldenliste, file attachment with a SHA-256, and nine `ultrafibu` commands. **Still open: the document editor, and multi-page PDF output** **A document's tax treatment is checked against itself and against the partner: a line that charges VAT while stating the customer owes it is refused at both printing and posting, and zero-rating a cross-border supply requires the customer's USt-IdNr. `SteuerschluesselFuerPartner` is what a tax dropdown binds to - it never suggests a key posting would refuse, and offers no default where the choice (goods or service) is one the program cannot make.** The entry form now exists as its own screen, with the tax dropdown per position, the reason under it and live totals. Still open there: the PDF preview beside the form for an uploaded receipt, creating a partner from the form, discounts, payment terms and templates, deleting a single position, and an account picker in place of a typed account number. | Done for the engine: a month entered, posted, paid, partly reversed, frozen and verified — 126 checks, including three lines of 33,33 EUR at 19 % owing 19,00 and a row edited with plain SQL being caught |
...
| **A8** *(server store ✅, migration gate ✅, concurrent numbering ✅; soft edit locks and local↔server transfer open)* | **Server mode**: `Store::OpenServer()` on the same schema and the same migrations as the local file, role enforcement, transactional number allocation, the migration gate. **The acceptance test forks two processes, and the first time it ran it produced 80 numbers of which 40 were distinct — a read-then-write counter, safe under SQLite and not under PostgreSQL. It now checks unique *and* gap-free, because a missing number is what an audit asks about.** Still open: optimistic locking with soft edit locks, and local↔server database transfer | Two clients on one PostgreSQL database allocating concurrently without a duplicate or a gap ✅ (`Tests/UltraFIBU/UltraFIBUServerTests.cpp`, run in CI) |
