...
| **A3** *(export ✅ for 21 and 20, import and 16 open)* | **DATEV**: `EXTF` Buchungsstapel and Kontenbeschriftungen, one file per calendar month, `WJ-Beginn` from the *Geschäftsjahr*, CP1252/CRLF, unsigned `Umsatz`, and `datev-pruefen` to check the shipped column definition against a real file. **Still open: import, category 16 (Debitoren/Kreditoren, ~240 columns, not guessed), and the BU-Schlüssel mapping** | The user's *Kanzlei* imports a stack without errors — the real acceptance test, which needs their file |
