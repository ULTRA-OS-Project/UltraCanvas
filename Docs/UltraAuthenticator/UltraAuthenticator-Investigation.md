...
(§2.2b), the OTP engine (§2.2d) and the app shell (§4, build order step 4)
are built and tested. In-memory QR decode (§2.2c) — and so the camera scan
flow — is the remaining functional gap; screen-capture protection (§2.2e)
remains impossible on X11.
...
Apps/UltraAuthenticator/
  ...
  AuthenticatorWindow.*        — account rows, live codes, 1 Hz countdown     [DONE]
