...
to storing seeds in the clear. When the vault is first created the password
is asked for twice and the two must match, because a typo in the one password
that cannot be recovered would otherwise become it; a strength bar under the
field shows what the vault will rest on while it is still cheap to change.
