...
- Markdown documents are checked as prose, not as markup: fenced and indented code blocks, inline `code`, link and image targets, autolinks, inline HTML, math and YAML front matter are all left alone, while the words around them — link text and image alt text included — are still checked.
- Spell checking is skipped for a file opened in Hex/Binary view, and follows the language you pick in the status bar.
- Requires a dictionary to be installed on the system (hunspell/myspell data, or whatever enchant finds). If none is present, switching the feature on says so instead of silently doing nothing.
