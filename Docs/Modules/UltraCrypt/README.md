...
- The codecs are declared in `UltraCanvasUtilsEncoding.h` (which
  `UltraCanvasUtils.h` includes) and compiled into a small `UltraCanvasEncoding`
  static library, because `UltraCanvasUtils.cpp` is platform glue — process
  launching, `<windows.h>` — that a headless module cannot compile. Both the
  UltraCanvas library and UltraCrypt link that library, so each symbol exists
  exactly once even in an application that links both.
...
```cpp
// UltraCanvasUtilsEncoding.h (also reachable via UltraCanvasUtils.h)
...
1. **Is BLAKE3-256 worth keeping as UCD v2 `SVLT` hash algorithm `1`, or should
   that field collapse to SHA-256 only**, consistent with the one-algorithm
   principle already applied to the cipher and KDF? The trade-offs, so the
   decision has its evidence in one place. Note what the hash is for: the
   `SVLT` creation hash covers the header, thumbnail, `SVLT` record and every
   ciphertext, is computed once at creation, is stored in the file **and**
   registered with the SuperVault service, and is recomputed by both the app
   and the service at open — so whatever is chosen must be implemented on the
   service side as well.

   | | SHA-256 only | Keep BLAKE3-256 as code point `1` |
   |---|---|---|
   | Availability | Already in libsodium (`crypto_hash_sha256`), already used by this module | In neither libsodium (which has BLAKE2b) nor OpenSSL 3.0; means vendoring the reference C implementation and registering a new third-party dependency |
   | Standardisation | FIPS 180-4 | None — a 2020 design with a single reference implementation |
   | Speed | Hardware-accelerated on every current x86 (SHA-NI) and ARMv8 CPU; the creation hash is computed once per document, so throughput is irrelevant here | Fastest *software* hash available, but that headline compares against software SHA-256, not accelerated SHA-256 |
   ...
   | Upgrade path | The record's version byte already allows a future algorithm by spec revision | Same version byte — reserving `1` today buys nothing that a revision would not |
   | Diversity from SHA-2, if ever wanted | — | BLAKE2b-256 gives the same property for free from libsodium; BLAKE3 is not the cheapest way to get it |

   Recommendation: collapse to SHA-256 only, remove or mark reserved code point
   `1` in `UCD-FileFormat-v2.md`, and rely on the version byte for any future
   change. Awaiting a ruling; no code depends on the answer today.
