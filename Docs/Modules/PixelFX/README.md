###PixelFX -- Professional Bitmap Processing for UltraCanvas
-----------------------
...
-   **File handling** -- a finished load stays in libvips' operation cache, which keeps the source file open and, for JPEG, memory-mapped; `ReleaseCachedFiles()` lets go of them. Call it before writing over a file the application read earlier -- Windows refuses to truncate a file that has a mapping open in the process, so without it saving an edited image back over itself fails
-   **Metadata** -- `Header::HasMetadata` / `ReadMetadata` / `MetadataToText` read what a file carries (EXIF, IPTC, XMP, ICC, the image's own geometry), tidy it for a person (tag names without their `exif-ifd0-` prefix, values without libvips' encoding suffix, IPTC and XMP decoded tag by tag, other binary blocks by size) and write it out as Markdown or plain text -- see [UltraCanvasMetadataDialog](../../UltraCanvas/UltraCanvasMetadataDialog.md)
----------------
### Why PixelFX?
