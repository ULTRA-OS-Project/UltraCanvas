...
| mDNS: `dn` differs between backends | ❌ Avahi and the Windows backend put the *unescaped* instance in `dn`; Bonjour passes through the escaped `fullname` it is given. Only cosmetic today — eSCL uses `dn` as a display fallback — but two platforms disagreeing about the same field will eventually catch someone. `Mdns::SplitInstanceName` is what macOS would use to match. |
