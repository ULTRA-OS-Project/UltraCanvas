...
Minified CSS — a whole stylesheet on one line — is scanned exactly. The text
area highlights one line at a time, so a line in a multi-line file that
opens no block of its own starts from a guess: a `{` on it makes it a
selector, a `;` or `}` makes it a declaration, and otherwise a `:` without a
trailing `,` does. The middle lines of a `/* ... */` comment are recognised by
their leading `*` or closing `*/`.
