...
- a **`TooltipContent`** value: an ordered list of structured blocks — bold
  title, two- or three-column table rows with optional color swatch and
  definable per-column alignment, bullet list items, free markup text and
  separators — laid out natively by the manager (defined in
  `UltraCanvasTooltipTypes.h`, included by the manager header).
...
content.AddTitle("Sales 2026-08")                       // bold, titleFontSize
       .AddRow("Europe", "1 204 t")                     // two-column table row
       .AddRow(Color(66,133,244,255), "Asia", "1 890 t") // ... with color swatch
       .AddRow("Africa", "312", "t")                    // three-column table row
       .AddRow(Color(15,157,88,255), "Americas", "986", "t")
...
- **Rows** form a table of two or three columns. `AddRow(label, value)` makes
  a two-column row, `AddRow(label, value, value2)` a three-column row; both
  have an overload taking a leading `Color` swatch. Column 1 is drawn in the
  muted `secondaryTextColor`, the value columns in `textColor`.
- **Column widths** are the columns' natural widths when the row fits. When it
  does not, the first column is capped at 55 % of the available width and the
  remaining columns share what is left in proportion to their natural widths;
  over-wide cells wrap inside their column. Surplus width — a wide `AddText`
  block, say — is spread evenly over the gaps, so the last column always ends
  flush with the tooltip's right edge. If any row carries a swatch, all rows
  shift by the swatch gutter so the columns stay aligned.
- **Two- and three-column rows are separate tables.** Each arity is measured
  into its own grid and aligned by its own setting, so a two-column "Total"
  row appended to a three-column table never disturbs that table's columns.
- **Column alignment** is definable per column, separately for the two- and
  three-column table (`TooltipColumnAlign::Left | Center | Right | Decimal`):

  ```cpp
  content.SetColumnAlignment(TooltipColumnAlign::Left,     // two-column rows
                             TooltipColumnAlign::Right);
  content.SetColumnAlignment(TooltipColumnAlign::Right,    // three-column rows
                             TooltipColumnAlign::Decimal,
                             TooltipColumnAlign::Left);
  ```

  Alignment places the cell inside its column box; a cell wide enough to wrap
  also lays its own lines out that way. These per-tooltip settings win over
  the style's `columnAlign2` / `columnAlign3`; because they are stored outside
  `styleOverride`, switching theme with `SetStyle()` leaves the alignment
  untouched. The defaults — label left, values right — reproduce the classic
  tooltip look.
- **`Decimal` aligns numbers on their separator** rather than on an edge, so a
  column of mixed precision reads as one number stack:

  ```
       1,204.50 €          <- Decimal            1,204.50 €   <- Right
         -17.05 €                                  -17.05 €
           0.9  €                                      0.9 €
          12    €                                       12 €
       1,200.35 €                                 1,200.35 €
  ...
  The anchor of a cell is the **last `.` or `,` followed by a digit**, so a
  thousands separator never wins over the real decimal mark (both `1,204.50`
  and `1.204,50` resolve correctly) and a trailing unit with a period cannot
  steal it. A cell with no separator anchors at its end, which puts an integer
  right where the separator column sits. The column's anchor is the widest
  integer part in it, and the column reserves the widest integer part *plus*
  the widest fractional part — so it can be wider than any single cell, and no
  cell is clipped to stay aligned. Keep the unit in its own column (the
  three-column form) rather than in the number, so the digits own the column.
  A `Decimal` cell too wide for its column falls back to `Right`.
...
```cpp
// Row cell placement; Decimal lines numbers up on their '.' or ','
enum class TooltipColumnAlign { Left, Center, Right, Decimal };
...
struct TooltipStyle {
    ...
    // Column alignment, index 0 = left-most column. Two- and three-column
    // rows are separate tables, so each has its own alignment vector.
    // Values: Left | Center | Right | Decimal
    std::array<TooltipColumnAlign, 2> columnAlign2 = {Left, Right};
    std::array<TooltipColumnAlign, 3> columnAlign3 = {Left, Right, Right};
