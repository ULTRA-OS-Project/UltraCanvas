...
A main topic can declare its own structure while the rest of the map keeps the
default — XMind's "combine structures" behaviour:

```cpp
map->GetTopic(branchId)->structureOverride = MindMapStructure::OrgChartDown;
```

The overridden branch is laid out locally first, and the main pass then reserves
exactly the space its bounding box needs, so an overridden branch never collides
with its siblings. Overrides apply to **depth-1 branches**; a nested override is
ignored.

### Animated re-layout

```cpp
map->Style().layoutAnimationMs = 220.0;   // 0 disables
```

Topics tween from their old positions to the new ones with an ease-out curve
after any layout change — collapse, structure switch, reparent. Re-triggering
mid-tween starts from where things currently appear rather than snapping back.
...
```cpp
map->Style().connectorAttach = MindMapConnectorAttach::Baseline;  // or Perimeter
map->SetTopicConnectorLabel(childId, "leads to");
map->Style().showConnectorLabels = true;
```

`Baseline` runs the stroke to the bottom edge of each node — the classic
hand-drawn convention — instead of to the nearest point on its outline.
A connector label belongs to the **child**, since a connector is implied by the
parent/child link rather than being an object of its own.
...
A callout is a floating topic that points at another topic with a leader line:

```cpp
std::string card = map->AddCallout(topicId, "Explanatory text", 520.0, -300.0);
map->Style().calloutLeaderDashed = true;
```

The leader runs between the two nodes' perimeters and ends in a small dot on the
target, so it reads as a reference rather than a branch. If the target is later
deleted the card survives; its pointer simply stops resolving.

### Application data

```cpp
map->SetTopicUserData(id, "external-key");
map->SetTopicAttribute(id, "owner", "alice");
std::string owner = map->GetTopicAttribute(id, "owner");
```

Neither the layout engine nor the element interprets these; both round-trip
through JSON.
...
**Ctrl+drag** from a topic to empty canvas creates a child at the drop point.
The modifier is required so that a plain drag always means "reparent" and never
silently creates a topic when it misses a target.
...
Rendering is **culled**: only topics intersecting the visible world region (plus
a margin, so partially-visible nodes and their indicators do not pop at the
edge) are drawn. Connectors and relationships are not culled — they are cheap
and often span the visible region.
...
background layer, callout cards, connector labels, note/link/marker
indicators), and a gallery for switching theme, structure and connector style at
runtime. The editable tab also shows a single branch overridden to an org chart
inside an otherwise balanced map.
