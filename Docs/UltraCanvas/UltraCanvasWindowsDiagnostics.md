...
| Symptom | Use |
|---|---|
| Windows says **"This app can't run on your PC"** | either — both check the file's PE header first, see [below](#this-app-cant-run-on-your-pc) |
...
the executable and the failing module. If there is no exit code either —
Windows showed *"This app can't run on your PC"* instead of starting anything —
see [that section](#this-app-cant-run-on-your-pc): the file itself is being
rejected, and the launchers' header check says why.
...
A blue dialog from Windows itself — *This app can't run on your PC. To find a
version for your PC, check with the software publisher.* (in Thai
*โปรแกรมนี้ไม่สามารถทำงานในพีซีของคุณได้*, in German *Diese App kann auf dem PC
nicht ausgeführt werden*) — is a different failure from everything else in this
document. It is `CreateProcess` failing with `ERROR_BAD_EXE_FORMAT` (193) or
`ERROR_EXE_MACHINE_TYPE_MISMATCH` (216): Windows read the file's PE header,
decided it cannot map it, and **never created a process**. So there is no exit
code, no crash, no event-log entry, no framework log and no DLL involved — the
executable itself is what Windows rejects, and every executable in the package
is rejected the same way. Nothing the application does at run time can cause
it, and neither can a missing DLL (that is `0xC0000135`) or a mismatched one
(`0xC000007B`, which shows a different dialog naming the application).

The header can be wrong in only a few ways, and all of them are visible without
running anything:

1. **Built for another CPU architecture.** CI produces an `x86_64` and an
   `arm64` Windows package from the same sources; the file lists are identical
   and only the PE header differs. The arm64 package on an ordinary x64 PC
   gives exactly this dialog for every `.exe` in it. (The reverse — the x86_64
   package on an ARM64 machine — runs under emulation on Windows 11 and gives
   this dialog on Windows 10, which cannot emulate x64.) Check the name of the
   ZIP you downloaded: `UCDemo-Windows-<version>-x86_64.zip` is the one for an
   Intel/AMD machine.
2. **A truncated or empty file.** A download that stopped early, or an
   extraction that gave up part-way (some tools stop silently on a long path
   or a name with characters they cannot handle), leaves an `.exe` whose
   header parses but whose sections run past the end of the file. Windows
   treats that as a bad format. A 0-byte file is the extreme case.
3. **A subsystem version newer than the installed Windows.** Not something
   the MinGW toolchain produces on its own (it links 5.02 or 6.00), but it is
   the check the loader makes, so the scripts make it too.

Both launchers now read the header before they do anything else and name the
reason:

```
=== Executable header ===
Built for   : ARM64 (PE32+), GUI subsystem 6.00, 11 sections
File size   : 21843456 bytes

[X] Windows will refuse to start this file with "This app can't run on your PC"
    (ERROR_BAD_EXE_FORMAT). It never becomes a process, so there is no exit
    code, no event-log entry and no log to read. The reason:
    - this executable is built for ARM64 but this machine is x64. It is the
      arm64 package; the one for this PC is the x86_64 package
...
the current build shows this dialog" sounds like a regression in the sources,
and the first instinct is to diff the two builds. Diff the two *files* first:
if the working one is x64 and the failing one ARM64, or the failing one is
shorter than its section table says, the sources were never involved. The
build itself guards against shipping such a file — `package-win.sh` runs
`scripts/verify-pe.sh` over every `.exe` and `.dll` before zipping and fails
the package if any is built for the wrong architecture, truncated, or has a
header Windows would not load — so a package that CI published contains
correct files, and what arrived on the machine is what to look at.
...
instructions this machine lacks)) at 0x00007FF8355D3793 in
C:\...\Ladybird\bin\lagom-gfx.dll. Windows 11 (10.0 build 26200)
This CPU: AMD Ryzen 5 5500U with Radeon Graphics (x86-64-v3) [SSE4.2 AVX AVX2
FMA BMI2 AES SHA]
Bytes at the fault: C4 E3 F9 CE 05 83 D1 03 00 00 C5 F9 D7 C0 3D FF
The binary was built for a CPU this one is not. Rebuild it with
-march=x86-64-v3 instead of -march=native (or /arch:AVX*). ...
```
...
The reporter prints four things that settle it between them:

- **the module** — which library was compiled too aggressively;
- **the x86-64 level** — the highest psABI microarchitecture level this machine
  satisfies. It is the actionable number, because `-march=x86-64-v<N>` means
  exactly that set: a build targeting it is guaranteed to run here;
- **the feature list** — read from CPUID, covering the extensions that actually
  break a `-march=native` build;
- **the bytes at the fault** — enough to identify the instruction.

### Reading the bytes

Do not stop at the prefix. `62` is EVEX and does imply AVX-512, but `C4`/`C5`
is VEX, which encodes far more than AVX and AVX2 — GFNI, VAES, VPCLMULQDQ and
BMI are all VEX-encoded. **A CPU can have every feature in the list and still
refuse a VEX instruction.** Decode it rather than guessing; any disassembler
will take raw bytes:

```sh
printf 'C4E3F9CE0583D1030000C5F9D7C0' | ... > f.bin   # bytes, hex, no spaces
objdump -D -b binary -m i386:x86-64 -M intel f.bin
```

The example above decodes to `vgf2p8affineqb xmm0, xmm0, [rip+0x3d183], 0` —
a **GFNI** instruction, on a CPU whose list says `AVX2` and does not say `GFNI`.
That is the whole bug, and reading `C4` as "AVX2, which this CPU has" would have
sent you looking in the wrong place.

### The three causes

1. **The build machine had a newer CPU than the target.** `-march=native` bakes
   in whatever the *builder's* processor supported. Build with the level the
   reporter names instead. Never ship `-march=native` binaries.
2. **An opt-in extension outside every `-march` level.** GFNI, VAES, VPCLMULQDQ
   and SHA belong to *no* `x86-64-v1..v4` level — no `-march=x86-64-vN` will
   ever emit them, and nothing but `-march=native` (or an explicit `-mgfni` and
   friends) picks them up. This is the nastiest version of the bug, because the
   target CPU can be modern and fully AVX2-capable and still fault. AMD added
   GFNI in Zen 4; Intel in Ice Lake and Tremont. A Zen 2 or Zen 3 laptop chip
   has AVX2 and no GFNI.
3. **The binary is x64 running under emulation on an ARM64 machine.** The
...
Picking a level: `-march=x86-64-v2` (SSE4.2, POPCNT) runs on anything that runs
Windows 10 or 11. `-march=x86-64-v3` adds AVX2, FMA and BMI2 — good performance,
but it excludes pre-2015 hardware, some current low-power parts and many virtual
machines. `v4` requires AVX-512 and is not a sensible shipping target. When in
doubt ship `v2`; when the reporter says `v3`, `v3` is safe *for that machine*.
