...
The Xara file format is the successor of the ArtWorks vector graphics file
format. ArtWorks is a powerful vector graphics editor for RISC OS, the
operating system of the 32-bit ARM machines — RISC OS was the first OS that
ran on ARM CPUs. Two things set ArtWorks apart: its ultra-fast CPU-based
vector graphics rendering, and its user-friendly user interface. It was later
ported to Windows, where it got the name Xara.

- [ArtWorks](https://en.wikipedia.org/wiki/ArtWorks) (Wikipedia)
- [Xara Designer Pro+](https://en.wikipedia.org/wiki/Xara_Designer_Pro%2B) (Wikipedia)

The demo's XAR page carries the same note, with both links clickable.
...
`Tests/XARFidelityTest` covers the other half of "parses fine, looks wrong":
it checks that the samples' colours, winding rules and text kerns resolve to
what Xara itself draws.

### How colours resolve

A `TAG_DEFINECOMPLEXCOLOUR` record opens with the RGB triple Xara itself puts
on screen for that colour — already run through the colour model and, for
Tint / Shade / Linked colours, through the whole parent chain — and that
triple is what the renderer paints. The record's colour model, type, parent
reference and four `<ColourDescription>` components are parsed alongside it as
the file's colour definition, for callers that edit or re-write colours rather
than paint them.

Do not repaint from those components. They are absolute coordinates only for
Normal colours; for a Shade they are two *signed* parameters relative to the
parent, so treating them as absolute drives the colour towards black. And even
for Normal colours they only ever reproduce the stored triple (RGB, HSV and
greyscale) or fail to (CMYK, whose screen appearance comes from Xara's
separation tables — the naive `(1-ink)(1-K)` conversion is up to 38/255 off on
the bundled samples).

### Two attributes that are easy to read wrong

**Winding rule.** Xara fills paths by the **even-odd** rule, and that is the
default here — a path with no `TAG_WINDINGRULE` record in scope still fills
the way Xara draws it. Do not assume non-zero: Xara writes each subpath of a
glyph outline in the same direction, so under non-zero a letter's counters
fill solid instead of punching through (the "B" and "9" on `Midget.xar`'s
number plate). Byte 0 on the record, the value both samples use throughout,
is the even-odd rule.

**Text kerns.** `TAG_TEXT_KERN`'s eight bytes are *not* a `Coord`. Only the
first INT32 is the kern, in millipoints, and it is horizontal; applying the
second as a Y offset puts a visible step in the middle of a line of text.

Known renderer gaps (parse succeeds, display is approximate): conical
gradients fall back to a linear ramp and diamond fills to a radial one (cairo
has neither primitive), fractal /
...
std::string xarFile1 = NormalizePath(GetResourcesDir() + "media/vector/XAR/Midget.xar");
