...
auto-placed in a reserved strip on the right of the element, vertically centred
and clear of the axis labels; pass an explicit element-local position to
override. The legend box is sized from the measured series names, and the same
width is reserved by the layout pass, so long names are neither clipped nor
...
3. **Layout** — computes `centerPoint` and `maxRadius` from the local bounds,
   reserving margin from the *measured* axis-label and legend text so long
   names cannot be clipped, and centring the ring in what is left
4. **Grid** — concentric rings + radial spokes (one per axis)
5. **Axis labels** — placed by the shared `PlaceShapeLabels()` solver
   (`UltraCanvasLabelPlacement.h`): each label prefers the side facing radially
   outward from its spoke, and moves to another side when that spot would fall
   under the legend, over a neighbouring label, or outside the element
