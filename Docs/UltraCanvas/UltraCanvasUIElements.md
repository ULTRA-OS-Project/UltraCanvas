...
Password fields also get a reveal control: an eye button inside the field, on
by default (`SetShowPasswordToggle(false)` removes it), and `SetPasswordRevealed()` drives the same state
