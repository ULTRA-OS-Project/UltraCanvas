...
| CDR | `CDRConverter` | ✓ (CDR plugin) | ✓ | Writing: version-7 RIFF targeting libcdr's parser layouts (no public spec exists). Solid fills, outline state, fill opacity; text/bitmaps skipped. Reading: the CDR plugin's libcdr parse, turned into SVG by librevenge's generator and read by `SVGConverter` — so it gets exactly what libcdr understands (the same engine LibreOffice uses), bitmaps included as inline images. What libcdr loses stays lost: in `media/vector/CDR/detailed.cdr` it drops the bitmaps' transparency and the rotation of four card images. First page only. Without `ULTRACANVAS_PLUGIN_CDR` (all Windows builds so far) `CanImport()` is false. |
