...
| CDR | `CDRConverter` | ✓ (CDR plugin) | ✓ | Writing: version-7 RIFF targeting libcdr's parser layouts (no public spec exists). Solid fills, outline state, fill opacity; text/bitmaps skipped. Reading: the CDR plugin's libcdr parse, turned into SVG by librevenge's generator and read by `SVGConverter` — so it gets exactly what libcdr understands (the same engine LibreOffice uses), bitmaps included as inline images. libcdr (0.1.7) reads a bitmap's colour image but not the 8-bit alpha mask CorelDRAW stores right after it (colour model 99); the importer reads the masks from the file itself (`content/data/Bitmaps.dat` in a ZIP-format CDR, inline in an older RIFF one) and re-embeds those bitmaps as RGBA PNG, so drop shadows and cut-out overlays keep their transparency. Bitmaps libcdr places no object for stay missing: `media/vector/CDR/detailed.cdr` holds six such CMYK bitmaps (leaves, waves, a shield's gloss). First page only. Without `ULTRACANVAS_PLUGIN_CDR` (all Windows builds so far) `CanImport()` is false. |
