...
- **Drill-down navigation:** double-click a node to descend into its subtree; navigate back to root.
...
The element hit-tests **what it draws** — the current level's children, laid out
by the active algorithm, whether or not they are leaves. So a grouped treemap
(one rectangle per group, with children only visible after drilling in) is just
as clickable as a flat one.

| Gesture | Effect |
|---|---|
| Left click on a rectangle | Selects it and fires `onNodeSelect(name, value)`. |
| Left click on the background | Clears the selection (`onNodeDeselect`). |
| Left double-click | Fires `onNodeDoubleClick`, then drills into the node when it has children (`onNodeDrillDown`). |
| Right click on a rectangle | Fires `onNodeRightClick(name)`. |
| Wheel / drag | Zoom and pan, when `SetInteractionOptions` enables them. |
