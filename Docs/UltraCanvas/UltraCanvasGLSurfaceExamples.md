...
| **Shaders** | `UltraCanvasGLShaderTab.cpp` | A shader playground: one full-screen triangle (emitted from `gl_VertexID`, no vertex buffer) driven entirely by animated fragment shaders — plasma, an SDF ray-marched scene with soft shadows, an animated Julia set, a polar tunnel, a parallax warp starfield, a Twigl/つぶやきGLSL "geek-mode" chaotic lattice one-liner, and a fragment-shader port of a p5.js noise-bloom sketch. The only inputs are `uTime` and `uResolution`. |
