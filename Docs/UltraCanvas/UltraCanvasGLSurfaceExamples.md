...
|-----|--------|----------------------|
| **3D Models** | `UltraCanvasGLModelsTab.cpp` | Loads complex Wavefront `.obj` meshes from `media/3D/` (torus knot, spring coil, ico-sphere) into the surface and renders them with a two-light Phong + rim shader. A `UltraCanvasGLSurface` subclass turns mouse drag into an orbit camera and the wheel into zoom; 4× MSAA smooths the silhouettes. |
| **Shaders** | `UltraCanvasGLShaderTab.cpp` | A shader playground: one full-screen triangle (emitted from `gl_VertexID`, no vertex buffer) driven entirely by animated fragment shaders. The list opens with three Twigl/つぶやきGLSL "geek-mode" one-liners — "Horizon" (a turbulent landscape under a glowing sky), "Protostar2" (a glowing volumetric core folded about a twisting axis) and "Plasma Orb" (a domain-warped cos field tinted by exp() gradients) — followed by plasma, an SDF ray-marched scene with soft shadows, an animated Julia set, a polar tunnel, a Twigl "Borg Sphere" lattice one-liner, a fragment-shader port of the "Ball Surface" p5.js sketch (four noise-warped rings of `F`-weighted white splats), the Twigl "Pulse" ray-fold (a per-pixel ray-fold accumulating tinted glow), "Fragments" (a marched tube with quantised sine turbulence) and "Mountains" (a log-polar fractal terrain with HSV-tinted glow). The base inputs are `uTime` and `uResolution`; selecting the Ball Surface, Pulse, Fragments or Circles effect also reveals live parameter sliders — Ball Surface brightness / splat radius, Pulse step scale / fold phase, Fragments fold-count / tube-radius, and Circles node-count / link-distance / ring-growth — pushed in as extra uniforms. "Circles" — an openFrameworks (C++) generative sketch — is also included; it is not a fragment shader but a 2D node-link network (180 Perlin-noise-placed nodes, links under 50px, rings sized by neighbour count), drawn with native 2D GL geometry through a custom render path in the same surface. "Alien Caterpillar" — a p5.js point-cloud sketch (10,000 parametric points animated over time) — is likewise drawn as `GL_POINTS` via the same custom render path. A "⛶" icon button overlaid on the canvas — or double-clicking the canvas — maximizes it over the whole tab (hiding the controls and source viewer); pressing Esc or a single click restores the layout. A read-only `UltraCanvasTextArea` under the canvas shows each effect's generating source (the original Twigl/p5.js snippet or closed-form formula as a header, followed by the live GLSL) with true GLSL syntax highlighting — a dedicated GLSL grammar (`vec*`/`mat*`/`sampler*` types and built-ins like `normalize`/`mix`/`texture`) was added to `UltraCanvasSyntaxTokenizer`. |
...
`media/3D/generate_models.py` and committed as `.obj` files.
