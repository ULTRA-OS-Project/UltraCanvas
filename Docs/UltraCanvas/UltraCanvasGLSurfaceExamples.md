...
| **Shaders** | `UltraCanvasGLShaderTab.cpp` | A shader playground: one full-screen triangle (emitted from `gl_VertexID`, no vertex buffer) driven entirely by animated fragment shaders — plasma, an SDF ray-marched scene with soft shadows, an animated Julia set, a polar tunnel, and a parallax warp starfield. The only inputs are `uTime` and `uResolution`. |
