...
| **Shaders** | `UltraCanvasGLShaderTab.cpp` | A shader playground: one full-screen triangle (emitted from `gl_VertexID`, no vertex buffer) driven entirely by animated fragment shaders — plasma, an SDF ray-marched scene with soft shadows, an animated Julia set, a polar tunnel, a parallax warp starfield, a Twigl/つぶやきGLSL "geek-mode" chaotic lattice one-liner, a numerically-integrated Rössler strange attractor (`dx/dt=−y−z`, `dy/dt=x+ay`, `dz/dt=b+z(x−c)`, `a=b=0.2`, `c=5.7`), and a fragment-shader port of the "Ball Surface" p5.js sketch (four noise-warped rings of `F`-weighted white splats). The base inputs are `uTime` and `uResolution`; selecting the Rössler or Ball Surface effect also reveals live parameter sliders — Rössler `a`/`b`/`c` (sweep `c` ≈ 4→9 to watch the attractor period-double into chaos) and Ball Surface brightness / splat radius — pushed in as extra uniforms. |
