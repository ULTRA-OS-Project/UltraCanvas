// Tests/UltraMessage/test_framework.h
// Tiny zero-dependency test framework for the UltraMessage suite. Each
...
#define TEST(name)                                                            \
    static void name();                                                       \
    static ::ultramsg_test::Reg reg_##name(#name, name);                       \
    static void name()
    ...
    do {                                                                      \
        if (!(expr)) {                                                        \
            ...
            oss__ << "REQUIRE(" #expr ") failed at " << __FILE__ << ":"       \
                  << __LINE__;                                                \
            throw ::ultramsg_test::Failure{oss__.str()};                       \
        }                                                                     \
    } while (0)
