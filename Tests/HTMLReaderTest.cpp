...
static void TestCssLength() {
    ...
    // Number shapes the two ParseNumber paths in CSSStyleSheet.cpp must agree
    // on - the fallback one delimits the number itself, and an 'e' that turns
    // out to be a unit rather than an exponent is where that goes wrong.
    ...
    CHECK(!CssLength::Parse("1e999px"));   // out of range for a float
