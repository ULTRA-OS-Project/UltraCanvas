// Tests/UltraWin/test_framework.h
// Tiny zero-dependency test framework for the UltraWin test suite.
// One TU per area (test_validation.cpp, test_manifest.cpp, ...). Each TEST(name)
...
inline int RunAllTests() {
    std::fprintf(stdout, "Running %zu UltraWin tests\n", Registry().size());
    for (auto& t : Registry()) {
...
#define TEST(name)                                                         \
    static void ultrawin_test_##name();                                    \
    static ultrawin_test::Reg ultrawin_test_##name##_reg{                  \
        #name, ultrawin_test_##name};                                      \
    static void ultrawin_test_##name()

#define REQUIRE(expr)                                                      \
    do {                                                                   \
        if (!(expr)) {                                                     \
            ultrawin_test::RecordFail(__FILE__, __LINE__, #expr);          \
            return;                                                        \
        }                                                                  \
    } while (0)
...
#define REQUIRE_EQ(a, b)                                                   \
    do {                                                                   \
        ...
        auto _y = (b);                                                     \
        if (!(_x == _y)) {                                                 \
            ultrawin_test::RecordFail(                                     \
                __FILE__, __LINE__,                                        \
                ultrawin_test::EqMessage(#a " == " #b, _x, _y));           \
            return;                                                        \
        }                                                                  \
    } while (0)
...
#define SKIP(reason)                                                       \
    do {                                                                   \
        ultrawin_test::RecordSkip(reason);                                 \
        return;                                                            \
    } while (0)
