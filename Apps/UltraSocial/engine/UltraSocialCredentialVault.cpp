// Apps/UltraSocial/engine/UltraSocialCredentialVault.cpp
// Same file backend as UltraMail's CredentialVault (kept in each app's own
// vault directory); both swap to the shared UltraVault module when it lands.
...
bool CredentialVault::Store(const std::string& accountId, const std::string& secret) {
    if (accountId.empty()) return false;
    auto key = LoadOrCreateKey(dir_);
    auto creds = LoadAll(dir_, key);
    creds[accountId] = secret;
    return SaveAll(dir_, key, creds);
}

bool CredentialVault::Retrieve(const std::string& accountId, std::string& out) const {
    out.clear();
    auto key = LoadOrCreateKey(dir_);
    auto creds = LoadAll(dir_, key);
    auto it = creds.find(accountId);
    if (it == creds.end()) return false;
    out = it->second;
    return true;
}

bool CredentialVault::Has(const std::string& accountId) const {
    std::string ignore;
    return Retrieve(accountId, ignore);
}

bool CredentialVault::Remove(const std::string& accountId) {
    auto key = LoadOrCreateKey(dir_);
    auto creds = LoadAll(dir_, key);
    auto it = creds.find(accountId);
    if (it == creds.end()) return false;
