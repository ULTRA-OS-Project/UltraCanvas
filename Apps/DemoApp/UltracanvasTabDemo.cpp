...
// ===== HELPER: CREATE SAMPLE TAB CONTENT =====
    std::shared_ptr<UltraCanvasContainer> CreateSampleTabContent(
            const std::string& id, int uid, const std::string& text, const Color& bgColor) {

        auto content = std::make_shared<UltraCanvasContainer>(id, uid, 0, 0, 750, 180);
...
//        modernTabs->SetActiveTabIndicatorColor(Color(33, 150, 243)); // Material Blue
        ...
        int tabIndex3 = featureTabs->AddTab("Settings", CreateSampleTabContent("Feature3", 7057,
                                                                               ...
                                                                               Color(250, 245, 255)));
        featureTabs->SetTabIcon(tabIndex3, "assets/icons/settings.png");
...
//        newTabATabs->SetNewTabButtonIcon("+");
        ...
        auto positionContainer = std::make_shared<UltraCanvasContainer>("PositionContainer", 7111, 40, yOffset, 800, 450);
        positionContainer->SetBackgroundColor(Colors::Transparent);

        // Bottom Position Tabs
        auto bottomTabs = std::make_shared<UltraCanvasTabbedContainer>("BottomTabs", 7112, 10, 10, 380, 200);
        ...
        bottomTabs->AddTab("Bottom 1", CreateSampleTabContent("Bottom1", 7113,
                                                              "Tabs at bottom\n(Terminal/Console style)", Color(250, 250, 250)));
        bottomTabs->AddTab("Bottom 2", CreateSampleTabContent("Bottom2", 7115,
                                                              "Useful for tool windows", Color(245, 250, 255)));
        bottomTabs->SetActiveTab(0);
        positionContainer->AddChild(bottomTabs);

        // Left Position Tabs
        auto leftTabs = std::make_shared<UltraCanvasTabbedContainer>("LeftTabs", 7116, 410, 10, 380, 200);
        leftTabs->SetTabPosition(TabPosition::Left);
        leftTabs->SetTabStyle(TabStyle::Modern);
        leftTabs->AddTab("Left 1", CreateSampleTabContent("Left1", 7117,
                                                          "Vertical tabs on left side", Color(250, 250, 250)));
        leftTabs->AddTab("Left 2", CreateSampleTabContent("Left2", 7119,
                                                          "Sidebar navigation style", Color(245, 250, 255)));
        leftTabs->SetActiveTab(0);
        positionContainer->AddChild(leftTabs);

        // Right Position Tabs
        auto rightTabs = std::make_shared<UltraCanvasTabbedContainer>("RightTabs", 7120, 10, 230, 780, 200);
        rightTabs->SetTabPosition(TabPosition::Right);
        rightTabs->SetTabStyle(TabStyle::Rounded);
        rightTabs->AddTab("Right Panel 1", CreateSampleTabContent("Right1", 7121,
