...
void UltraCanvasClipboardItem::OnEvent(const UCEvent& event) {
    lastMousePos = Point2D(event.x, event.y);
    
    if (event.type == UCEventType::MouseDown && event.button == MouseButton::Left) {
...
void UltraCanvasClipboardManager::CreateClipboardWindow() {
    ...
    // ✅ UPDATED: Use new event callback names
    clipboardWindow->onWindowBlurred = [this]() {
        HideClipboardWindow();
    };
    
    // Set up key event handler for escape key
    clipboardWindow->SetEventHandler([this](const UCEvent& event) -> bool {
        if (event.type == UCEventType::KeyDown && event.key.code == UCKeys::Escape) {
            HideClipboardWindow();
            return true;
        }
        return false;
...
void UltraCanvasClipboardManager::RegisterGlobalHotkey() {
    // ✅ UPDATED: Use new hotkey registration API
    UltraCanvasKeyboardManager::RegisterHotkey(
        UCKeys::P, // P key
    ...
    std::cout << "Registered ALT+P hotkey for clipboard manager" << std::endl;
}
